...
# FrequentPatternForgottenBasketPreventer
class XMT:
    def __init__(self, new2old: Dict[int, int]):
        ...
        # Parameters
        self.min_purchases = 5  # Minimum purchases for interval analysis
        self.large_basket_threshold = 10  # Minimum items for a "large basket"
        self.repurchase_window = 2  # Days to look for repurchases
        self.seasonal_boost = 1.5  # Boost factor for seasonal items
        self.max_interval_multiplier = 3.0  # Maximum multiplier of avg interval to consider
        self.max_absolute_days = 90  # Maximum absolute days since last purchase to consider
    ...
    def _calculate_item_frequencies(self):
        """Calculate base frequency of items"""
    ...
    def _analyze_purchase_intervals(self):
        """Analyze time intervals between purchases"""
    ...
    def build_model(self, customer_train_set):
        """Build the prediction model using customer training data"""
    ...
    def predict_f(self, current_basket: Set[str], basket_date: str, max_final_predictions: int = 3) -> Tuple[
        Set[str], List[str]]:
        """Predict forgotten items based on current basket"""
        if self.state != 'built':
            raise Exception('Model not built, prediction not available')

        # Map old category names to new IDs for internal processing
        ...
        # First predict X't (expected full basket)
        x_t_prime_scores = {}
        for item in self.item_frequencies:
            # Check if item is too old to consider
            if item in self.last_purchase_dates and item in self.avg_intervals:
                ...
                # Skip items that haven't been purchased in too long
        ...
        # Score potential missing items with additional evidence
        missing_scores = {}
        for item in potential_missing:
            # Start with the X't score
            score = x_t_prime_scores[item]
            evidence_boost = 0

            # Strong co-purchase patterns with current basket
            co_purchase_boost = 0
            for basket_item in current_basket_mapped:
                if basket_item in self.co_purchase_counts[item]:
                    ...
                    if co_occurrence > 5:  # Minimum co-occurrences threshold
                        co_purchase_boost += 0.2  # Boost for strong co-purchase pattern
            evidence_boost += co_purchase_boost
            ...
            missing_scores[item] = score + evidence_boost

        # Select final predictions and convert back to category names
        prediction_items = sorted(
            missing_scores.keys(),
            key=missing_scores.get,
            reverse=True
        ...
        for item in prediction_items:
            ...
            if item in self.last_purchase_dates and item in self.avg_intervals:
                ...
                # Only include timing information if it's within reasonable bounds
            ...
            # Co-purchase patterns with current basket
            relevant_items = []
            for basket_item in current_basket_mapped:
                if basket_item in self.co_purchase_counts[item]:
            ...
            if item in self.repurchase_patterns and self.total_repurchase_opportunities[item] > 0:
                ...
                if repurchase_rate > 0.3:
                    explanation += f"3. Often repurchased soon after large shopping trips "
