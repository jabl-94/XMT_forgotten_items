...
def estimate_nbr_bins(x):
    ...
    # Handle cases where k_fd is infinity or NaN
...
class TARSTree(object):
    ...
    def preprocessing(self, baskets):
        ...
        for item in list(self.item_periods.keys()):
        ...
        if self.item_min_period_support_parent is None:
            ...
            nbr_bins = np.round(estimate_nbr_bins(list(self.item_estimated_nbr_periods.values())))
            _, bins = np.histogram(list(self.item_estimated_nbr_periods.values()), bins=nbr_bins)
            ...
            for item in list(self.item_period_support.keys()):
                estimated_nbr_periods = self.item_estimated_nbr_periods[item]
        ...
        if self.item_min_expected_espp_parent is None:
            ...
            nbr_bins = np.round(estimate_nbr_bins(list(self.item_nbr_periods.values())))
            _, bins = np.histogram(list(self.item_nbr_periods.values()), bins=nbr_bins)
    ...
    def build_fptree(self, baskets, root_value, root_count, root_timeseries):
        ...
        for t0, basket_id0 in enumerate(sorted_basket_ids):
            ...
            if t0 < len(sorted_basket_ids) - 1:
                ...
                sorted_items = [i[0] for i in sorted(iter(sorted_items.items()),
                                                     key=lambda k_v: (k_v[1], -k_v[0][0][0], -k_v[0][1][0]),
                                                     reverse=True)]
    ...
    def generate_pattern_list(self):
        ...
        patterns = {}
        items = list(self.single_item_timeseries.keys())
    ...
    def mine_sub_trees(self, rec_dept, max_rec_dept, patterns_subset=None, nbr_patterns=None,
        ...
        if rec_dept < max_rec_dept:
            ...
            mining_order = [i[0] for i in sorted(iter(self.item_support.items()),
                                                 key=lambda k_v1: (k_v1[1], -k_v1[0][0][0], -k_v1[0][1][0]),
            ...
            if patterns_subset is not None:
                ...
                if nbr_patterns is not None:
                    ...
                    if len(subset_item_support) > 3:
                        ...
                        min_sup_interesting = np.percentile(list(subset_item_support.values()), 90)
                        for item in mining_order:
                ...
                if len(self.item_timeseries[item]) > 1:
                    ...
                    if espp >= -min_espp:
                        ...
                        if len(subtree.root.children) > 0:
                            ...
                            for pattern in list(subtree_patterns.keys()):
                                new_pattern = pattern
...
def calculate_intervals_support(freq_rec_patterns, tree):
    ...
    for rp, info in freq_rec_patterns.items():
...
def calcualte_active_rp(customer_data, rp_intervals_support, day_of_next_purchase, max_rp_len=3):
    ...
    for t0, basket_id0 in enumerate(sorted_basket_ids):
        ...
        if t0 < len(sorted_basket_ids) - 1:
            ...
            for i in range(1, max_rp_len):
                for subset_tail in itertools.combinations(basket0, i):
                    for j in range(1, max_rp_len):
...
def main():
    ...
    if dataset == 'tafeng':
        path = 'C:/Users/javie/PycharmProjects/TARS/dataset/'
