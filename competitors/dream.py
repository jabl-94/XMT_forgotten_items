...
from competitors.dream_model.model.rnnwithbpr_theano_czy import RCnnWithBpr
...
def f1_score5(pre_y, test_y, default_none):
    tp = 0.0
    fn = 0.0
    fp = 0.0
    for y in pre_y:
...
def ndcg5(pre_y, test_y, default_none):
    ...
    for test in test_y:
        if test != -1:
            length_test += 1
    s = 0
    for i in range(length_pre):
        if pre_y[i] != default_none:
...
class DREAM:
    ...
    def __negative(self, users_rank, num_items):
        negtargets = []
        for i in range(len(users_rank)):
            negtargets.append([])
            for j in range(len(users_rank[i])):
                if users_rank[i][j] == -1:
                    k = -1
                else:
    ...
    def __pad_data(self, baskets, max_basket_size):
        ...
        for u in range(len(baskets)):
            for b in range(len(baskets[u])):
    ...
    def build_model(self, baskets, folder):
        ...
        for i in range(self.n_epoch):
            ...
            for user_id in range(len(self.user_basket_dream)):
                ...
                if i > 0:
                    ...
                    if int(self.n_item) in set(recall_6_y):
                        recall_5_y = []
                        default_num += 1
                        for recall in recall_6_y:
                            if int(self.n_item) != recall:
            ...
            # timing display
            if self.verbose:
                print (datetime.datetime.now(), 'Epoch %s, loss: %s' % (i, cost), 'Time: ', datetime.datetime.now()-start_time)
