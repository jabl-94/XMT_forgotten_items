...
def update_f1_scores(tuple_dict, new_k, new_f1_score):
    if new_k in tuple_dict:
        tuple_dict[new_k].append(new_f1_score)
    else:
        tuple_dict[new_k] = [new_f1_score]

    return tuple_dict


def calculate_median_f1_scores(tuple_dict):
    return [(k, median(scores)) for k, scores in tuple_dict.items()]
...
def calculate_aggregate(values):
    agg_measures = {
        ...
        'cov': 1.0 * np.mean(values) / np.std(values) if np.std(values) != 0 else 0.0,
