...
from contenders.new_tars_ibp_predictor import *
from contenders.new_ibp import *
...
def predict_with_model(model, customer_id, customer_data, next_baskets, new2old,
    ...
    for i in range(len(next_baskets) - 1):
        ...
        if isinstance(model, Top):
            ...
            # Predict using HRM
            pred_start_time = datetime.datetime.now()
            pred_basket = model.predict(user_id=customer_id, last_basket=list(last_basket.keys()), pred_length=pred_length)
            pred_end_time = datetime.datetime.now()
            customer_id = real_id
            print(customer_id)
            # Remap predicted items
            ...
            pred_basket = model.predict(customer_data, day_of_next_purchase, nbr_patterns=None, pred_length=pred_length)
            pred_basket = set(cod_mkt_cat2name.get(new2old[item], new2old[item]) for item in pred_basket)
        ...
        elif isinstance(model, IntervalBasedForgottenBasketPredictor):
            pred_start_time = datetime.datetime.now()
            pred_basket, explanations = model.predict_f(customer_id,
                                                        current_basket_date,
                                                        current_basket,
                                                        k=pred_length)
            pred_end_time = datetime.datetime.now()
            ...
            for item, explanation in zip(pred_basket, explanations):
                # print(f"Predicted forgotten item: {item}")
                # print(explanation)
                print()
    ...
    if model_name == "hrm":
        ...
        for customer_id in customers_train_set:
            ...
            if model_name == 'top':
                ...
                model.build_model(customer_train_set)
            elif model_name == "IFBP":
                model = IntervalBasedForgottenBasketPredictor(new2old)
                print('eyoo')
                model.build_model(customer_train_set)
                print('eyoo 2')
            elif model_name == "TFBP":
